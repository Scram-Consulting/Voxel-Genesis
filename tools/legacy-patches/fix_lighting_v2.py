...
# Encontrar la línea donde empieza propagateSunlight
start_idx = None
for i, line in enumerate(lines):
    if 'void propagateSunlight()' in line:
        start_idx = i - 1  # Incluir el comentario anterior
        break

if start_idx is None:
    print("ERROR: No se encontro propagateSunlight()")
    exit(1)

# Encontrar donde termina (el siguiente método después del cout final)
end_idx = None
for i in range(start_idx + 10, len(lines)):
    if 'Sunlight propagation completada!' in lines[i]:
        # Buscar el cierre de la función (siguiente línea con solo '}')
...
if end_idx is None:
    print("ERROR: No se encontro el fin de propagateSunlight()")
    exit(1)

print(f"Reemplazando lineas {start_idx+1} a {end_idx}")

# Nueva implementación optimizada
new_function = """    // Propagar SUNLIGHT horizontalmente (después de skylight vertical)
...
print("OK: propagateSunlight optimizada!")
print("  - Agregado visited set")
print("  - Limite de 1M iterations")
